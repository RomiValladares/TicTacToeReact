...
const Square = ({ value, isWinning, isXNext, disabled, onClick }: any) => (
    <button
        onClick={onClick}
        disabled={disabled}
        className={`
            group relative z-10 w-24 h-24 sm:w-28 sm:h-28 rounded-3xl border flex items-center justify-center transition-all duration-300
            ${isWinning
                ? 'ring-4 ring-(--primary)/50 bg-(--primary)/10 scale-105'
                : 'bg-(--text-main)/5 border-(--text-main)/10 hover:bg-(--text-main)/15'}
        `}
    >
        {value === 'X' && <X size={44} strokeWidth={3} className="text-(--primary) drop-shadow-[0_0_8px_var(--primary)]" />}
        {value === 'O' && <Circle size={38} strokeWidth={3} className="text-(--secondary) drop-shadow-[0_0_8px_var(--secondary)]" />}
