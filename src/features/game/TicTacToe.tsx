...
const ThemeButton = ({ type, current, onSelect }: { type: 'neon' | 'sunset' | 'classic', current: string, onSelect: (t: any) => void }) => (
    <button
        ...
        className={`w-5 h-5 rounded-full border-2 transition-all ${current === type ? 'scale-110 border-[var(--text-main)]' : 'border-transparent opacity-40 hover:opacity-100'}`}
...
const Square = ({ value, isWinning, isXNext, disabled, onClick }: { value: string | null, isWinning: boolean | undefined, isXNext: boolean, disabled: boolean, onClick: () => void }) => (
    <button
        ...
        className={`
            ...
            ${isWinning ? 'ring-4 ring-[var(--primary)]/50 bg-[var(--primary)]/10 scale-105' : 'hover:bg-[var(--text-main)]/5'}
        `}
    >
        {value === 'X' && <X size={44} strokeWidth={3} className="text-[var(--primary)] animate-in zoom-in" />}
        {value === 'O' && <Circle size={38} strokeWidth={3} className="text-[var(--secondary)] animate-in zoom-in" />}
...
const ScoreCard = ({ label, score, colorClass, isLast = false }: { label: string, score: number, colorClass: string, isLast?: boolean }) => (
    <div className={`flex flex-col items-center ${!isLast ? 'border-r border-[var(--text-main)]/10' : ''}`}>
        <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest mb-1">{label}</span>
...
const GameStatus = ({ winner, isAiThinking, isXNext }: { winner: string | null, isAiThinking: boolean, isXNext: boolean }) => {
    if (winner === 'draw') {
        return (
            <div className="h-10 flex items-center">
                <span className="text-sm font-black tracking-[0.3em] text-[var(--text-muted)] uppercase border-y border-white/5 py-1">
                    Settled in a Draw
                </span>
            </div>
        );
    ...
    if (winner) {
        return (
            <div className="h-10 flex items-center gap-3 text-3xl font-black tracking-tighter text-[var(--primary)] drop-shadow-[0_0_15px_var(--primary)]">
    ...
    return (
        <div className="h-10 flex items-center">
            <p className="text-[var(--text-muted)] font-bold tracking-widest text-xs uppercase opacity-80">
...
export const TicTacToe: React.FC = () => {
    ...
    return (
        <div data-theme={theme} className="flex flex-col items-center min-h-screen bg-[var(--bg-main)] text-[var(--text-main)] p-6 transition-colors duration-500 font-sans">

            <header className="w-full max-w-md flex flex-col items-center mb-10 mt-4">
                <nav className="flex gap-3 mb-8 bg-[var(--text-main)]/5 p-1.5 rounded-full border border-[var(--text-main)]/10 backdrop-blur-md">
            ...
            <footer className="w-full max-w-xs flex flex-col items-center gap-8">
                <div className="grid grid-cols-3 w-full bg-[var(--text-main)]/5 rounded-3xl border border-[var(--text-main)]/10 p-4 backdrop-blur-md">
                    <ScoreCard label="Player" score={scores.X} colorClass="text-[var(--primary)]" />
                    <ScoreCard label="Draws" score={scores.draws} colorClass="text-[var(--text-main)]" />
                    <ScoreCard label="AI" score={scores.O} colorClass="text-[var(--secondary)]" isLast />
                </div>
                ...
                <button
                    ...
                    className="flex items-center gap-3 px-12 py-4 bg-[var(--btn-bg)] text-[var(--btn-text)] font-black rounded-2xl hover:scale-105 active:scale-95 transition-all shadow-xl"
                >
                    <RotateCcw size={18} />
                    REMATCH
                </button>
